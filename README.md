...
The browser fetches a fresh credential set with a six-second timeout, refreshes it every 35 minutes and reapplies it to active peer connections. If the Worker or Cloudflare TURN API is unavailable, the room continues with Cloudflare STUN instead of failing startup. Credentials are held only in page memory and both Worker and client use `no-store`.

WebRTC prefetches a small candidate pool, tries direct host and STUN paths, and falls back to Cloudflare TURN over UDP, TCP, or TLS. Non-trickle signaling proceeds as soon as a usable relay candidate plus a short 250 ms TCP/TLS settlement window is available instead of waiting for every ICE route to finish. The relay sees network metadata and encrypted WebRTC packets, but WebRTC DTLS and optional room-level encryption remain end to end; the TURN server does not receive plaintext project content.
...
During active calls and file transfers, p2p-share requests the Screen Wake Lock API where supported. Returning to a visible tab refreshes presence and expiring TURN credentials without destroying healthy routes. An eight-second overlay health check repairs missing neighbors locally, initial ICE failures trigger immediate localized repair, and a failed or 12-second timed-out attempt escalates the next route to relay-only ICE. Cloudflare’s browser-blocked port 53 alternatives are removed from non-trickle offers, while TURN/TLS 443 and TURN/TCP 80 are prioritized for corporate networks. Transient WebRTC disconnections receive a 15-second grace period, simultaneous repair offers use deterministic glare resolution, and candidate pooling is bounded to two prefetched generations per peer to avoid excessive allocations in large rooms. Mobile operating systems can still suspend browser execution completely; a static PWA or service worker cannot guarantee a live WebRTC connection while the OS has suspended the page.
