...
await joinPage.waitForURL(/#room=A1b2C3/, { timeout: 15_000 });
results.push({ label: "join", hash: new globalThis.URL(joinPage.url()).hash });
...
await createPage.waitForURL(/#room=[A-Za-z0-9]{6}$/, { timeout: 15_000 });
const createdRoomId = new globalThis.URL(createPage.url()).hash.split("=")[1];
...
await directPage.goto("http://127.0.0.1:5173/#room=DirectRoom42", { waitUntil: "domcontentloaded" });
